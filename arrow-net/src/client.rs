...
use arrow_protocol::{packets::{PacketKind, common::status, types::Gamemode, version_specific::types::{v754::{BiomeEffects, BiomeRegistry, BiomeRegistryEntry, BiomeProperties, DimensionCodec, DimensionRegistry, DimensionRegistryEntry, DimensionType}}}, serde::varint::VarInt};
    ...
    async fn status(mut self) {
        ...
        let response_data = status::ResponseData {
            ...
            players: status::PlayerData {
                ...
                online: SERVER.read().await.get_online_player_count() as i32,
                sample: Vec::new(),
            },
            description: status::DescriptionData {
                text: String::from("§0Hello §4world"),
            },
    ...
    async fn login(mut self) {
        ...
        if SERVER.read().await.has_uuid(&uuid).await {
            ...
            SERVER
                .write()
                .await
                .add_player(Arc::new(RwLock::new(Player::new(uuid, name, self)))).await;
        }
    }
        ...
        let dimension_codec = DimensionCodec {
            ...
            biome_registry: BiomeRegistry {
                ...
                value: vec![BiomeRegistryEntry {
                    name: String::from("minecraft:ocean"),
                    id: 0,
                    element: BiomeProperties {
                        precipitation: String::from("none"),
                        depth: 1.5,
                        temperature: 1.0,
                        scale: 1.225,
                        downfall: 0.5,
                        category: String::from("ocean"),
