...
use self::{common::*, error::PacketError, types::Gamemode, version_specific::types as version_types};
use crate::serde::de::Deserializer;

/// All packets that have the same data over all versions.
pub mod common;
/// The version specific packets
pub mod version_specific;
/// The error module for packets.
pub mod error;
/// all common type definitions are found here
pub mod types;
...
impl PacketKind {
    ...
    pub fn into_packet(self, protocol_version: i32) -> Result<Box<dyn Packet>, PacketError> {
        ...
        if protocol_version >= 754 {
            match self {
                JoinGame {
                    ...
                    is_debug,
                    is_flat,
                } => return Ok(Box::new(version_specific::play::v754::clientbound::JoinGame::new(
                    entity_id,
                    is_hardcore,
                    gamemode as u8,
        ...
        match self {
            ...
            _ => Err(PacketError::NoMatchingPacketFound)
        }
    }
